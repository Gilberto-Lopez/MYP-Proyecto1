...
public abstract class ArbolBinario<T> implements Coleccion<T> {
    ...
    /* Metodo auxiliar recursivo para profundidad(). */
    private int profundidad(Vertice v, int p){
	if(v == null)
	    return p - 1;
	else if(v.izquierdo == null && v.derecho == null)
	    return p;
	else
	    return Math.max(profundidad(v.izquierdo, p + 1),
			    profundidad(v.derecho, p + 1));
    }
	...
	if(vertice == null || elemento.equals(vertice.elemento))
	    ...
	    if(v == null)
		v = busca(vertice.derecho, elemento);
	    return v;
	}
    }
